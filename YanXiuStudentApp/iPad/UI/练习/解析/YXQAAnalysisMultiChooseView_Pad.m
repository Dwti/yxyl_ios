//
//  YXQAAnalysisMultiChooseView_Pad.m
//  YanXiuStudentApp
//
...
@interface YXQAAnalysisMultiChooseView_Pad()<YXKnowledgePointViewDelegate,YXQAAnalysisReportErrorViewDelegate,YXQAAnalysisUnfoldDelegate>
@property (nonatomic, assign) NSInteger superRowCount;
@property (nonatomic, strong) NSMutableArray *analysisDataArray;

@end

@implementation YXQAAnalysisMultiChooseView_Pad
...
- (void)setupAnalysisContent{
    ...
    if (!isEmpty(self.data.answerCompare)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateMultiChoose analysisType:YXAnalysisCurrentStatus]) {
    ...
    if (!isEmpty(self.data.globalStatis)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateMultiChoose analysisType:YXAnalysisStatistic]) {
            YXQAAnalysisItem *item = [[YXQAAnalysisItem alloc]init];
    ...
    if (!isEmpty(self.data.difficulty)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateMultiChoose analysisType:YXAnalysisDifficulty]) {
            YXQAAnalysisItem *item = [[YXQAAnalysisItem alloc]init];
    ...
    if (!isEmpty(self.data.analysis)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateMultiChoose analysisType:YXAnalysisAnalysis]) {
            YXQAAnalysisItem *item = [[YXQAAnalysisItem alloc]init];
    ...
    if (!isEmpty(self.data.knowledgePoints)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateMultiChoose analysisType:YXAnalysisKnowledgePoint]) {
            YXQAAnalysisItem *item = [[YXQAAnalysisItem alloc]init];
