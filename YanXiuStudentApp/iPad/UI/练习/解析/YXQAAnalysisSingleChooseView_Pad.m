//
//  YXQAAnalysisSingleChooseView_Pad.m
//  YanXiuStudentApp
//
...
@interface YXQAAnalysisSingleChooseView_Pad()<YXKnowledgePointViewDelegate,YXQAAnalysisReportErrorViewDelegate,YXQAAnalysisUnfoldDelegate>
@property (nonatomic, assign) NSInteger superRowCount;
@property (nonatomic, strong) NSMutableArray *analysisDataArray;
@end

@implementation YXQAAnalysisSingleChooseView_Pad
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (myRowIndex < 0) {
        YXQAChooseAnswerCell_Pad *cell = (YXQAChooseAnswerCell_Pad *)[super tableView:tableView cellForRowAtIndexPath:indexPath];
        if ([cell isKindOfClass:[YXQAChooseAnswerCell_Pad class]]) {
            cell.bChoosed = FALSE;
            cell.markType = YXQAChooseMarkNone;
            if (!self.analysisDataHidden) {
                NSInteger answerIndex = indexPath.row - 1;
                if ([self.data.myAnswers[answerIndex] boolValue]) {
                    cell.bChoosed = YES;
                    cell.markType = YXQAChooseMarkWrong;
                }
                if ([self.data.correctAnswers[answerIndex] boolValue]) {
                    cell.markType = YXQAChooseMarkCorrect;
                }
            }
        }
        return cell;
...
- (void)setupAnalysisContent{
    ...
    if (!isEmpty(self.data.answerCompare)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateSingleChoose analysisType:YXAnalysisCurrentStatus]) {
    ...
    if (!isEmpty(self.data.globalStatis)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateSingleChoose analysisType:YXAnalysisStatistic]) {
            YXQAAnalysisItem *item = [[YXQAAnalysisItem alloc]init];
    ...
    if (!isEmpty(self.data.difficulty)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateSingleChoose analysisType:YXAnalysisDifficulty]) {
            YXQAAnalysisItem *item = [[YXQAAnalysisItem alloc]init];
    ...
    if (!isEmpty(self.data.analysis)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateSingleChoose analysisType:YXAnalysisAnalysis]) {
            YXQAAnalysisItem *item = [[YXQAAnalysisItem alloc]init];
    ...
    if (!isEmpty(self.data.knowledgePoints)) {
        if ([self.analysisDataDelegate shouldShowAnalysisDataWithQAItemType:YXQATemplateSingleChoose analysisType:YXAnalysisKnowledgePoint]) {
            YXQAAnalysisItem *item = [[YXQAAnalysisItem alloc]init];
