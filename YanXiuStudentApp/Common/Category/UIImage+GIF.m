...
+ (UIImage *)nyx_animatedGIFNamed:(NSString *)name {
    ...
    if (scale > 1.0f) {
        ...
        if (data) {
            return [UIImage nyx_animatedGIFWithData:data];
        }
...
+ (UIImage *)nyx_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self nyx_frameDurationAtIndex:i source:source];
...
+ (float)nyx_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
