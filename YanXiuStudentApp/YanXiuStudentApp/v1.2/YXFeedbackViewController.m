...
- (void)setupViews
{
    ...
    bgView.image = [UIImage imageNamed:@"我的背景"];
...
//            [self yx_showToast:@"达到500字上限！"];
    ...
    [[self.viewModel.feedbackCommand errors] subscribeNext: ^(NSError* e) {
        @strongify(self);
        [self yx_stopLoading];
        [self yx_showToast:@"发送失败！"];
    }];
    [[self.viewModel.feedbackCommand executionSignals] subscribeNext:^(RACSignal* signal) {
