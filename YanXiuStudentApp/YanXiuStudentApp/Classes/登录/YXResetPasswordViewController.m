...
@interface YXResetPasswordViewController ()
...
{
    if ([self.passwordInput.text yx_isValidString]
        && [self.confirmPasswordInput.text yx_isValidString]) {
...
- (YXPasswordInputView *)confirmPasswordInput
{
    if (!_confirmPasswordInput) {
        ...
        [_confirmPasswordInput setShowPassword:YES];
    }
    ...
    [LoginUtils verifyPasswordFormat:password completeBlock:^(BOOL isEmpty, BOOL formatIsCorrect) {
        ...
        switch (self.type) {
            ...
            case YXPasswordOperationTypeFirstSet:
                ...
                [self presentSearchClassViewController];
                break;
        }
    }];
}
...
- (void)presentEditProfileViewController
{
    ...
    [vc setMobile:self.phoneNumber password:self.passwordInput.text];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)presentSearchClassViewController {
