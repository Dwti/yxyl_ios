...
@interface YXPhotoListViewController () <
...
@implementation YXPhotoListViewController
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    YXPhotoModel *model = self.photoArray[indexPath.row];
    ...
    cell.selectHandle = ^(YXPhotoModel * yxModel, UIButton * sender){
        ...
        if (yxModel.isSelect) {
            ...
            [alertView addButtonWithTitle:@"我知道了" action:nil];
            [alertView show];
        }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat width = (CGRectGetWidth(self.view.frame) - 15 * 4)/3;
...
- (id <MWPhoto>)photoBrowser:(MWPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index {
    if (index < [self.albumViewModel getCurrentPhotoListArrayCount])
    {
        YXPhotoModel * model = self.albumViewModel.currentPhotoListArray[index];
        return model;
    }
    return nil;
}
...
- (id <MWPhoto>)photoBrowser:(MWPhotoBrowser *)photoBrowser thumbPhotoAtIndex:(NSUInteger)index {
    if (index < [self.albumViewModel getCurrentPhotoListArrayCount])
        return self.albumViewModel.currentPhotoListArray[index];
    return nil;
}
...
- (BOOL)photoBrowser:(MWPhotoBrowser *)photoBrowser isPhotoSelectedAtIndex:(NSUInteger)index {
    if (index < [self.albumViewModel getCurrentPhotoListArrayCount])
        return [self.albumViewModel.currentPhotoListArray[index] isSelect];
    return NO;
}
