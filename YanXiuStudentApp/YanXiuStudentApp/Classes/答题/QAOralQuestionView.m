...
@property (nonatomic, assign) BOOL enteredForeground;
@end
...
- (void)setupOralRecordView {
    self.oralRecordView = [[QAOralRecordView alloc] initWithState:(self.oriData.questionType == YXQAItemOralRead || self.oriData.questionType == YXQAItemOralComposition) ? QAOralRecordViewStateNormal : QAOralRecordViewStateDisabled];
    ...
    if (indexPath.row == 0) {
        ...
        cell.audioBtnBlock = ^(UIButton *sender, NSURL *audioUrl) {
            ...
            if (![self.audioUrl isEqual:audioUrl]) {
                ...
                if (self.enteredForeground) {
                    self.player.videoUrl = audioUrl;
                }
            }
        };
...
- (void)leaveForeground {
    ...
    if (self.oralRecordView.player.state != PlayerView_State_Finished) {
        [self.oralRecordView.player pause];
        self.oralRecordView.player.state = PlayerView_State_Finished;
        
    }
    if (self.oralRecordView.recordViewState == QAOralRecordViewStateRecording) {
        [self.oralRecordView.recognizer cancel];
    }
}

- (void)enterForeground {
    self.enteredForeground = YES;
    if (self.data.answerState == YXAnswerStateNotAnswer && self.audioUrl) {
        self.player.videoUrl = self.audioUrl;
    }
